...
p2_process <- list(
  ...
  # Load each tif file and save as a terra object in a qs file
  tar_target(p2_sedclass_terraqs, {
    save_as_terra(
      in_file = p1_hs_sedclass_tif_info$tif_fn,
      out_file = sprintf('2_process/tmp/%s', 
                         gsub('.tif', '.qs', 
                              basename(p1_hs_sedclass_tif_info$tif_fn))))
  },
  ...
  tar_target(p2_classification_summary_overall,
             summarize_class_pct_overall(p2_sedclass_terraqs),
             pattern = map(p2_sedclass_terraqs)),
  
  # Summarize all raster classes as a time series per outlet polygon
  tar_target(p2_sediment_crs, terra::crs(load_terraqs(p2_sedclass_terraqs[1]))),
  ...
  tar_target(p2_classification_summary_byOutlet,
             summarize_class_pct_byOutlet(p2_sedclass_terraqs, p2_outlet_list_sf),
             pattern = map(p2_sedclass_terraqs)),
